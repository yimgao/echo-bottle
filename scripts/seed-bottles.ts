/**
 * Seed script to populate the public bottle pool with initial data
 * 
 * Usage:
 *   npm run seed:bottles
 * 
 * Note: This script uses Firebase Admin SDK to bypass security rules.
 * Make sure you have a service account key file or are running in an
...
if (fs.existsSync(serviceAccountPath)) {
  ...
  // This works when running with gcloud auth application-default login
...
async function seedBottles() {
  ...
  const poolCollection = db.collection('artifacts').doc(appId).collection('public').doc('data').collection('pool_bottles');

  try {
    // Step 1: Delete all existing bottles
    console.log('🗑️  Step 1: Deleting existing bottles...');
    const existingBottles = await poolCollection.get();
    ...
    for (const bottle of SEED_DATA) {
      const docRef = poolCollection.doc();
      batch.set(docRef, {
        content: bottle.text,
        type: bottle.mood,
